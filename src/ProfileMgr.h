...
public:
    ...
    struct ConfigData
    {
        ...
        uint32_t threadStrategy = 0;    // Thread hijacking strategy
        uint32_t timingTrigger = 0;     // Injection timing trigger
        ...
        bool bypassPPL = false;         // Bypass Protected Process Light
        bool unhookNtdll = false;       // Unhook ntdll before injection
        bool cloakModule = false;       // Hide module from PEB
        bool spoofStack = false;        // Spoof call stack
        bool antiDebug = false;         // Enable anti-debug measures
        bool perfMonitor = false;       // Enable performance monitoring
    };
