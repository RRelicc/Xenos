...
INT_PTR MainDlg::OnRemoveImage( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    if (idx >= 0 && idx < static_cast<int>(_images.size()))
    {
...
INT_PTR MainDlg::OnExecute( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    if (_injectionThread.joinable())
        _injectionThread.join();
    _injectionThread = std::thread( &MainDlg::Inject, this );
    return TRUE;
}
...
INT_PTR MainDlg::OnProtectSelf( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    ...
    if (blackbone::Driver().loaded())
    {
        NTSTATUS unlinkStatus = blackbone::Driver().UnlinkHandleTable( GetCurrentProcessId() );
        if (!NT_SUCCESS( unlinkStatus ) && NT_SUCCESS( status ))
            status = unlinkStatus;
    }
