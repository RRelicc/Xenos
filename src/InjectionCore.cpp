...
namespace
{
    constexpr DWORD PROCESS_WAIT_SLEEP_MS = 100;
    constexpr DWORD APC_EXECUTION_WAIT_MS = 100;
    constexpr SIZE_T INIT_ARG_BUFFER_SIZE = 0x1000;
}
...
NTSTATUS InjectionCore::GetTargetProcess( InjectContext& context, PROCESS_INFORMATION& pi )
{
    ...
    else if (context.cfg.processMode == NewProcess)
    {
        ...
        std::wstring cmdLine = context.cfg.procCmdLine;
        if (!CreateProcessW( context.procPath.c_str(), cmdLine.data(),
        ...
        if (context.cfg.krnHandle)
        {
            ...
            status = RetryHelper::Retry( [&]() {
                return _process.Attach( pi.dwProcessId, PROCESS_QUERY_LIMITED_INFORMATION );
            }, 3, 100, RetryHelper::IsRetryableStatus );

            if (NT_SUCCESS( status ))
            {
                status = RetryHelper::Retry( [&]() {
                    return blackbone::Driver().PromoteHandle(
                        GetCurrentProcessId(),
        ...
        {
            status = RetryHelper::Retry( [&]() {
                return _process.Attach( pi.dwProcessId );
            }, 3, 100, RetryHelper::IsRetryableStatus );
        }
    ...
    {
        if (context.cfg.bypassPPL && PPLBypass::IsPPLSupported())
        {
            xlog::Normal( "Attempting PPL bypass for PID %d", context.pid );

            HANDLE hBypass = nullptr;

            if (Win11Compat::IsVBSEnabled() || Win11Compat::IsHVCIEnabled())
            {
                xlog::Warning( "PPL bypass not possible with VBS/HVCI enabled" );
                status = PPLBypass::BypassPPLWithVBSCheck( context.pid, &hBypass );
            }
            else
            {
                status = PPLBypass::BypassPPL( context.pid, &hBypass );
            }

            if (NT_SUCCESS( status ) && hBypass)
            {
                _process.Attach( context.pid );
                xlog::Normal( "PPL bypass successful" );
            }
            else
            {
                xlog::Warning( "PPL bypass failed, status 0x%X", status );
            }
        }

        // Escalate handle access rights through driver
        if (context.cfg.krnHandle && !_process.valid())
        {
            xlog::Normal( "Escalating process handle access" );

            status = RetryHelper::Retry( [&]() {
                return _process.Attach( context.pid, PROCESS_QUERY_LIMITED_INFORMATION );
            }, 3, 100, RetryHelper::IsRetryableStatus );
        ...
        {
            status = RetryHelper::Retry( [&]() {
                return _process.Attach( context.pid );
            }, 3, 100, RetryHelper::IsRetryableStatus );
        }
        ...
        // Unhook ntdll if requested
        if (context.cfg.unhookNtdll)
        {
            xlog::Normal( "Unhooking ntdll.dll" );
            status = HookEvasion::UnhookModule( _process, L"ntdll.dll" );

            if (!NT_SUCCESS( status ))
                xlog::Warning( "Failed to unhook ntdll.dll, status 0x%X", status );
            else
                xlog::Normal( "ntdll.dll unhooked successfully" );
        }

        // Injection timing control
        if (context.cfg.timingTrigger != 0)
        {
            InjectionTiming::TimingConfig timingCfg;
            timingCfg.trigger = static_cast<InjectionTiming::Trigger>(context.cfg.timingTrigger);
            timingCfg.delayMs = context.cfg.delay;

            xlog::Normal( "Waiting for injection trigger: %d", timingCfg.trigger );
            status = InjectionTiming::WaitForCondition( _process, timingCfg );

            if (!NT_SUCCESS( status ))
                xlog::Warning( "Timing trigger wait failed, status 0x%X", status );
        }
    }
...
NTSTATUS InjectionCore::ValidateContext( InjectContext& context, const blackbone::pe::PEImage& img )
{
    ...
    if (context.cfg.injectMode == Kernel_DriverMap)
    {
        ...
        {
            Message::ShowError( _hMainDlg, L"Kernel injection is not supported with VBS/HVCI enabled.\nPlease disable Virtualization Based Security in Windows settings." );
    ...
    if (CheckDependencies( img, missing ) != STATUS_SUCCESS && !missing.empty())
    {
        std::wstring msg = L"Warning: Image '" + img.name() + L"' has missing dependencies:\n";
        for (const auto& dll : missing)
            msg += L"  - " + dll + L"\n";
        msg += L"\nInjection may fail. Continue anyway?";

        if (IDNO == MessageBoxW( _hMainDlg, msg.c_str(), L"Missing Dependencies", MB_YESNO | MB_ICONWARNING ))
...
/// <returns>Error code</returns>
NTSTATUS InjectionCore::CheckDependencies( const blackbone::pe::PEImage& img, std::vector<std::wstring>& missing )
{
    missing.clear();
    ...
    auto& imports = const_cast<blackbone::pe::PEImage&>(img).GetImports();
    auto& modules = _process.modules();

    for (auto& import : imports)
    {
        const std::wstring& dllName = import.first;

        if (!modules.GetModule( dllName, blackbone::LdrList, blackbone::mt_default ))
        {
            wchar_t sysPath[MAX_PATH] = { 0 };
            GetSystemDirectoryW( sysPath, MAX_PATH );

            std::wstring fullPath = std::wstring( sysPath ) + L"\\" + dllName;
            if (GetFileAttributesW( fullPath.c_str() ) == INVALID_FILE_ATTRIBUTES)
                missing.push_back( dllName );
        }
    }

    return missing.empty() ? STATUS_SUCCESS : STATUS_DLL_NOT_FOUND;
}
...
NTSTATUS InjectionCore::InjectMultiple( InjectContext* pContext )
{
    ...
    {
        NTSTATUS imgStatus = InjectSingle( *pContext, *img );
        if (!NT_SUCCESS( imgStatus ) && NT_SUCCESS( status ))
            status = imgStatus;
    ...
    //
    blackbone::Handle hThreadGuard( pi.hThread );
    blackbone::Handle hProcessGuard( pi.hProcess );

    if (pi.hThread && status == STATUS_SUCCESS)
        ResumeThread( pi.hThread );
...
NTSTATUS InjectionCore::InjectSingle( InjectContext& context, blackbone::pe::PEImage& img )
{
    ...
    ScopedTimer timer( "InjectSingle" );

    if (Win11Compat::IsWindows11OrGreater())
    {
        xlog::Normal( "Windows 11 detected (Build %d), using enhanced evasion", Win11Compat::GetBuildNumber() );

        if (Win11Compat::RequiresEnhancedEvasion())
        {
            xlog::Normal( "VBS/HVCI detected, some kernel-mode features may be restricted" );
        }

        if (!Win11Compat::IsProcessElevated())
        {
            xlog::Warning( "Process not elevated, injection may fail" );
        }

        if (!Win11Compat::HasSeDebugPrivilege())
        {
            xlog::Warning( "SeDebugPrivilege not available" );
            Win11Compat::EnableSeDebugPrivilege();
        }
    }

    if (context.cfg.antiDebug && AntiDebug::IsProcessBeingDebugged( _process ))
    {
        xlog::Warning( "Target process is being debugged" );
        AntiDebug::DisableDebuggerAttach( _process );
        AntiDebug::ClearDebugPort( _process );
    }
    ...
    {
        xlog::Normal( "Searching for thread to hijack using strategy %d", context.cfg.threadStrategy );

        auto strategy = static_cast<ThreadHijacking::SelectionStrategy>(context.cfg.threadStrategy);
        pThread = ThreadHijacking::SelectThread( _process, strategy );
        ...
        xlog::Normal( "Selected thread ID: %d", pThread->id() );
    }
            ...
            case Reflective:
                {
                    ...
                    status = ReflectiveLoader::InjectReflective( _process, img.path(), "ReflectiveLoader", &moduleBase );
                    if (!NT_SUCCESS( status ))
                        xlog::Error( "Failed to inject image using reflective loader, status: 0x%X", status );
                }
                break;

            case Reflective_SRDI:
                {
                    uint64_t result = 0;
                    std::string exportFunc = blackbone::Utils::WstringToUTF8( context.cfg.initRoutine );
                    status = ReflectiveLoader::InjectReflectiveSRDI( _process, img.path(), exportFunc, "", &result );
                    if (!NT_SUCCESS( status ))
                        xlog::Error( "Failed to inject image using SRDI, status: 0x%X", status );
                }
                break;
    ...
    else if (!NT_SUCCESS( status ))
    {
        ...
        swprintf_s( errBuf, ARRAYSIZE( errBuf ), L"Failed to inject image '%ls'.\nError code 0x%X", img.path().c_str(), status );
    ...
    // Module cloaking
    if (NT_SUCCESS( status ) && mod && context.cfg.cloakModule)
    {
        xlog::Normal( "Applying module cloaking" );

        if (context.cfg.erasePE)
            ModuleCloaking::ErasePEHeader( _process, mod->baseAddress );

        if (context.cfg.unlink)
            ModuleCloaking::UnlinkFromPEB( _process, mod->baseAddress );

        ModuleCloaking::RandomizeSectionNames( _process, mod->baseAddress );
    }

    if (context.cfg.perfMonitor)
    {
        auto report = PerformanceMonitor::Instance().GetReport();
        xlog::Normal( "%s", report.c_str() );
    }
    ...
    )
{
    ...
    {
        if (context.cfg.spoofStack && pThread)
        {
            xlog::Normal( "Using stack spoofing for injection" );

            auto validAddrs = CallstackWalker::GetValidReturnAddresses( _process, 5 );
            if (!validAddrs.empty())
            {
                auto ntdll = _process.modules().GetModule( L"ntdll.dll" );
                if (ntdll)
                {
                    auto ldrLoadDllResult = _process.modules().GetExport( ntdll, "LdrLoadDll" );
                    if (ldrLoadDllResult.success())
                    {
                        auto ldrLoadDll = ldrLoadDllResult.result();
                        StackSpoofer::ExecuteWithSpoofedStack(
                            _process,
                            ldrLoadDll.procAddress,
                            validAddrs[0],
                            {}
                            );
                    }
    ...
    )
{
    ...
    if (!context.cfg.initRoutine.empty() && !img.pureIL() && context.cfg.injectMode < Kernel_Thread)
    {
        ...
        if (pThread == nullptr)
        {
            auto argMem = _process.memory().Allocate( INIT_ARG_BUFFER_SIZE, PAGE_READWRITE );
            if (!argMem)
        ...
        {
            blackbone::RemoteFunction<fnInitRoutine> pfn( _process, reinterpret_cast<fnInitRoutine>(fnPtr) );
